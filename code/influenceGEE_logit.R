...
MSOM.GEElogit <- function(object, x)
{ # gradient-type influence measures for a single observation (i.e 'patients')
  ID <- object$id
  len <- table(ID)
  n <- length(len)
  N <- nrow(x)
